...
public class ArchiveOauth2AuthorizationServerConfiguration {
    ...
    @Bean
    Oauth2TokenStoreService redisOauth2TokenStoreService(RedisConnectionFactory redisConnectionFactory, Oauth2ServerProperties oauth2ServerProperties) {
        final RedisOauth2TokenStoreService tokenStoreService = new RedisOauth2TokenStoreService(redisConnectionFactory, oauth2ServerProperties);
    ...
    AuthenticationSuccessHandler authenticationSuccessHandler(ObjectMapper objectMapper, Oauth2TokenStoreService tokenStoreService, Oauth2ServerProperties properties) {
        final Oauth2TokenCreationService tokenCreationService = new DefaultOauth2TokenCreationService(tokenStoreService, properties);
    ...
    Oauth2AuthenticationServerSecurityConfig oauth2AuthenticationServerConfig(
        ...
        Oauth2ServerProperties oauth2ServerProperties, */AuthenticationSuccessHandler authenticationSuccessHandler, AuthenticationFailureHandler authenticationFailureHandler) {
        ...
        return new UserDetailsService() {
            ...
            public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(String.format("登录[loadUserByUsername]处理共耗时：%s ms", (System.currentTimeMillis() - start)));
                }
                return user;
            }
        };
    ...
    PasswordEncoder passwordEncoder() {
        ...
        final PasswordEncoder d = new org.springframework.security.crypto.password.MessageDigestPasswordEncoder("SHA-256");
    ...
    @Bean
    Oauth2LogoutHandler oauth2LogoutHandler(Oauth2TokenStoreService oauth2TokenStoreService, BearerTokenResolver bearerTokenResolver) {
    ...
    @Bean
    Oauth2ServerConfigService oauth2ServerConfigService(RedisConnectionFactory redisConnectionFactory, Oauth2ServerProperties oauth2ServerProperties) {

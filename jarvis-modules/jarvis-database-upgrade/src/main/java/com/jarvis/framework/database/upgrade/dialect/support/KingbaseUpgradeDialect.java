...
public class KingbaseUpgradeDialect extends AbstractUpgradeDialect {
    ...
    public Collection<String> getCreateTableSql(CreateTable model) {
        ...
        for (final Column column : model.getColumns()) {
            ...
            if (!column.isNullable() || BaseColumnConstant.ID.equalsIgnoreCase(column.getColumnName())) {
                sb.append(" NOT NULL");
            }
    ...
    public Collection<String> getModifyColumnSql(ModifyColumn model) {
        ...
        for (final Column column : model.getColumns()) {
            ...
            /*if (column.isNullable()) {
                sqls.add(
                    String.format("ALTER TABLE %s ALTER COLUMN %s DROP NOT NULL", tableName, column.getColumnName()));
            } else {
                sqls.add(
                    String.format("ALTER TABLE %s ALTER COLUMN %s SET NOT NULL", tableName, column.getColumnName()));
            }*/
    ...
    private String getAddColumnSql(String tableName, Column column) {
        ...
        sb.append("ALTER TABLE ").append(tableName).append(" ADD COLUMN ").append(column.getColumnName())
